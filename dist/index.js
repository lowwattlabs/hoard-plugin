...
function resolveMemoryDir(configMemoryDir) {
    ...
    if (blocked.some(d => resolved === d)) {
        throw new Error(`Blocked: HOARD_DIR resolves to "${resolved}", which is a system directory. ` +
    ...
    if (resolved === resolve(process.env.HOME || "/home")) {
        throw new Error(`Blocked: HOARD_DIR resolves to home directory "${resolved}". ` +
        ...
        tool({
            ...
            async execute({ action, key, content, pattern, confirm }, config) {
                ...
                switch (action) {
                    case "store": {
                        if (!key || !content)
                            throw new Error("store requires key and content");
                    ...
                    case "retrieve": {
                        if (!key)
                            throw new Error("retrieve requires key");
                        const path = safePath(HOARD_DIR, key);
